...
object RDDTempData extends App {
  ...
  }.cache() // saves this data to memory, otherwise every other call will re-read from the file

  val maxTemp = data.map(_.tmax).max
  val hotDays = data.filter(_.tmax == maxTemp)
  // println(hotDays.count) befor caling collect to make sure its not a huge amout of data - forces an action
